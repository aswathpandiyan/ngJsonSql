var app = angular.module('myapp', ['ngJsonSql']);

app.controller('MainCtrl', function($scope, ngJsonSql) {
...
$scope.sql ="select title,url from json.channel.items where (category=='javascript' || category=='vista') order by title,category asc limit 3";
  $scope.result = ngJsonSql.query($scope.sql, $scope.jsondata);
